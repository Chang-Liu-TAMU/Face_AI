...
from modules.utils.helpers import to_chunks, colorize_log, \
                            validate_max_size, reproject_points


#face data representation
...
class Detector:
    def __init__(self, device: str = 'cuda', det_name: str = 'retinaface_r50_v1', max_size=None,
        ...
        self.retina = get_model(det_name, backend_name=backend_name, force_fp16=force_fp16, im_size=max_size,
                                root_dir=root_dir, download_model=download_model, triton_uri=triton_uri,
...
class FaceAnalysis:
    def __init__(self, det_name: str = 'retinaface_r50_v1', rec_name: str = 'arcface_r100_v1',
                 ...
                 backend_name: str = 'trt', force_fp16: bool = False, triton_uri=None, root_dir: str ='/models',
        ...
        self.det_model = Detector(det_name=det_name, device=device, max_size=self.max_size,
                                  ...
                                  force_fp16=force_fp16, triton_uri=triton_uri, root_dir=root_dir, download_model=download_model)
        ...
        if rec_name is not None:
            self.rec_model = get_model(rec_name, backend_name=backend_name, force_fp16=force_fp16,
                                       ...
                                       download_model=download_model, triton_uri=triton_uri)
    ...
    # generate faces with complete info
    def process_faces(self, faces: List[Face], extract_embedding: bool = True, extract_ga: bool = True,
        ...
        """
        given faces with detection info,
        generate faces with complete info
        """
    ...
    # return: List[List[Faces]]
    async def get_async(self, images, extract_embedding: bool = True, extract_ga: bool = True, detect_masks: bool = True,
        ...
        if len(faces_by_img) == 1:
            return faces_by_img[0]
        return faces_by_img



    def get(self, images, extract_embedding: bool = True, extract_ga: bool = True, detect_masks: bool = True,
                  return_face_data: bool = True, max_size: List[int] = None, threshold: float = 0.6,
                  mask_thresh: float = 0.89,
                  limit_faces: int = 0):
        """
...
if __name__ == "__main__":
    model = FaceAnalysis(det_name='retinaface_r50_v1', rec_name="glintr100",
                         ga_name="genderage_v1", mask_detector='mask_detector',
                         device='cuda', max_size=None, max_rec_batch_size=1, max_det_batch_size=1,
                        backend_name='trt', force_fp16=False, triton_uri=None, root_dir='/models')
    print(model)
    ...
    image = cv2.imread("../test/k/image_test.jpg")
    faces = asyncio.get_event_loop().run_until_complete(model.get_async([image]))
    # print(faces)
    drawn_image = model.draw_faces(image, faces)
    cv2.imwrite("../test/k/drawn604.jpg", drawn_image)