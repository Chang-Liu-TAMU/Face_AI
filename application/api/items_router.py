...
async def items_test(
    ...
    if settings.REDIS_ENABLED:  # Do not support redis now
        # 测试redis使用
    ...
    test_sql = "SELECT user_id,nickname,authority_id,is_active,create_time,update_time \
                from sys_user WHERE id>=:id AND is_delete=0"
